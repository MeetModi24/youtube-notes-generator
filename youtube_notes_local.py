...
No API key required - uses local Claude installation via DevBar
"""
...
class LocalClaudeAnalyzer:
    """Uses local Claude CLI (DevBar/AWS Bedrock) to analyze content"""
    ...
    def _find_claude_command(self) -> str:
        """Find the claude command path"""
        # Try common locations
        possible_paths = [
            'claude',  # In PATH
            '/usr/local/bin/claude',
            str(Path.home() / '.devbar' / 'bin' / 'claude'),
        ]
        ...
        for cmd in possible_paths:
            try:
                ...
                if result.returncode == 0:
                    print(f"✓ Found Claude CLI at: {cmd}")
                    return cmd
            except (FileNotFoundError, subprocess.TimeoutExpired):
                continue

        raise RuntimeError(
            "Claude CLI not found. Make sure DevBar is installed and 'claude' command is in PATH.\n"
            "Try: which claude"
        )

    def analyze_content(self, transcript: str, metadata: Dict, style: str = "academic") -> Dict:
        """Analyze transcript and generate structured notes using local Claude"""
...
Transcript:
{transcript[:15000]}  # Limit transcript length for processing
...
IMPORTANT: Return ONLY the structured notes content. Do not add any preamble or meta-commentary."""
        ...
        try:
            ...
            # Call Claude CLI
            result = subprocess.run(
                [self.claude_cmd, 'ask', '-f', prompt_file],
                capture_output=True,
                text=True,
            ...
            if result.returncode == 0:
                ...
                print(f"Error from Claude CLI: {result.stderr}")
                return {
                    'raw_content': f"# Error generating notes\n\nFailed to analyze content. Error: {result.stderr}",
                    'diagrams': [],
                }
            ...
            return {
                'raw_content': "# Error generating notes\n\nAnalysis timed out.",
                'diagrams': [],
            }
        except Exception as e:
    ...
    def generate_diagram_spec(self, diagram_description: str, diagram_type: str) -> str:
        """Generate Mermaid diagram specification using local Claude"""
...
*Generated automatically by YouTube Notes Generator (Local Version)*
"""
...
*Generated using YouTube Notes Generator (Local Version - DevBar/AWS Bedrock)*
"""
...
class YouTubeNotesGenerator:
    ...
    def __init__(self, repo_path: str, output_dir: str):
        ...
        self.content_analyzer = LocalClaudeAnalyzer()
    ...
    def process_video(self, video_url: str, lesson_number: int, style: str = "academic") -> Dict:
        ...
        # Analyze content
        print("3. Analyzing content with local Claude...")
        ...
        print("5. Creating diagram placeholders...")
        placeholder_diagram = """flowchart TD
    A[Key Concept] --> B[Sub-concept 1]
    A --> C[Sub-concept 2]
    B --> D[Example]
    C --> E[Application]
    ...
    style C fill:#fff3e0"""
        ...
        diagram_file = diagrams_path / "01-concept-overview.mmd"
        diagram_file.write_text(placeholder_diagram)
...
def main():
    parser = argparse.ArgumentParser(
        description="Transform YouTube videos into comprehensive learning notes (Local Version - DevBar)"
    )
    ...
    print("YouTube Notes Generator - Local Version (DevBar/AWS Bedrock)")
    print("No API key required - using your local Claude setup\n")

    # Initialize generator
    generator = YouTubeNotesGenerator(args.repo_path, args.output_dir)
